...
module.exports.buildConfig = function buildConfig(params) {
  ...
  // Asset URLs used to resolve through npm workspace links. Keep their source
  // package paths explicit without changing TypeScript's public API aliases.
  const importAliases = {
    "@bitwarden/components/src": path.resolve(__dirname, "libs/components/src"),
    "@bitwarden/angular/src": path.resolve(__dirname, "libs/angular/src"),
  ...
  const mainConfig = {
    name: "main",
    // VaultMesh: never resolve loaders from the parent workspace.
    ...
    entry: {
      ...
      "content/vaultmesh-native-fill": path.resolve(__dirname, "src/vaultmesh/bootstrap-native-fill.ts"),
        ...
        rules: [
          {
            test: /\.tsx?$/,
            loader: "ts-loader",
            // Use the same runtime entry set as Angular, not the test/Storybook tsconfig.
            options: { configFile: path.resolve(__dirname, params.tsConfig) },
          },
        ],
      },
      cache:
  ...
  const remoteEntries = new Set(["popup/polyfills", "popup/main", "content/vaultmesh-native-fill", "background"]);
  mainConfig.entry = Object.fromEntries(Object.entries(mainConfig.entry).filter(([name]) => remoteEntries.has(name)));
  mainConfig.plugins = mainConfig.plugins.filter((plugin) => !(plugin instanceof HtmlWebpackPlugin)
    || ["popup/index.html", "background.html"].includes(plugin.userOptions.filename));
  return configs;
};
