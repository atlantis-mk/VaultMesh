...
export class CollectAutofillContentService implements CollectAutofillContentServiceInterface {
  ...
  constructor(
    ...
    localHeuristicsOnly = false,
  ) {
    this.formFieldQueryString = this.buildFormFieldQueryString();
    // VaultMesh has no cloud account/remote targeting-rule configuration.
    this.attributeSettingsFetched = localHeuristicsOnly
      ? Promise.resolve() : this.fetchAndSetBitwardenAttributeSettings();
    if (localHeuristicsOnly) this.pageTargetingRules = null;
