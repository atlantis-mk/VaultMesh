...
export class AutofillOverlayContentService implements AutofillOverlayContentServiceInterface {
  ...
  /** Remote-vault target ownership uses the same native submit-button search. */
  async getLocalFormScope(element: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement): Promise<HTMLElement | null> {
    if (element.form) return element.form;
    let parent = element.parentElement;
    while (parent && !["BODY", "HTML"].includes(parent.tagName)) {
      if (await this.findSubmitButton(parent)) return parent;
      parent = parent.parentElement;
    }
    const fallback = element.parentElement;
    return fallback && !["BODY", "HTML"].includes(fallback.tagName) ? fallback : null;
  }
