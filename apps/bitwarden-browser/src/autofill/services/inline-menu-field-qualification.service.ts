...
import { AutofillScriptGenerator as AutofillService } from "./autofill-script-generator";
...
export class InlineMenuFieldQualificationService implements InlineMenuFieldQualificationServiceInterface {
  ...
  constructor(localTotpCapability?: boolean) {
    if (localTotpCapability !== undefined) {
      this.premiumEnabled = localTotpCapability;
      return;
    }
  ...
  private isUsernameFieldForLoginForm(
    ...
    if (!parentForm) {
      ...
      // Before defaulting to login, consult the tie-breaker.
      if (isAmbiguousFieldNonLogin(field, pageDetails)) {
        return false;
      }
      return true;
    }
