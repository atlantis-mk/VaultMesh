...
class InsertAutofillContentService implements InsertAutofillContentServiceInterface {
  ...
  constructor(
    ...
    private readonly fillGuard?: (element: FormFieldElement) => boolean,
    private readonly didWrite?: (element: FormFieldElement) => void,
    private readonly beforeAction?: () => Promise<boolean>,
  ) {}
  ...
  async fillForm(fillScript: AutofillScript, showAnimations: boolean = true) {
    ...
    for (let index = 0; index < fillScript.script.length; index++) {
      if (this.beforeAction && !(await this.beforeAction())) return;
  ...
  private runFillScriptAction = ([action, opid, value]: FillScript): Promise<void> => {
    ...
    return new Promise((resolve) =>
      setTimeout(() => {
        const target = this.collectAutofillContentService.getAutofillFieldElementByOpid(opid);
        if (this.fillGuard && (!target || !this.fillGuard(target))) { resolve(); return; }
  ...
  private insertValueIntoField(element: FormFieldElement | null, value: string) {
    if (!element || !value || (this.fillGuard && !this.fillGuard(element))) {
      return;
    }
    ...
    this.handleInsertValueAndTriggerSimulatedEvents(element, () => this.setElementValue(element, value));
  }

  private setElementValue(element: FormFieldElement, value: string): void {
    if (this.fillGuard && element instanceof HTMLInputElement) {
  ...
  private handleInsertValueAndTriggerSimulatedEvents(
    ...
    // Click/focus/keyboard handlers can synchronously navigate or move a field.
    if (this.fillGuard && !this.fillGuard(element)) return;
    valueChangeCallback();
    this.didWrite?.(element);
  ...
  private triggerPreInsertEventsOnElement(element: FormFieldElement): void {
    ...
    if ("value" in element && initialElementValue !== element.value && (!this.fillGuard || this.fillGuard(element))) {
      this.setElementValue(element, initialElementValue);
    }
  }
  ...
  private triggerPostInsertEventsOnElement(element: FormFieldElement): void {
    ...
    if ("value" in element && autofilledValue !== element.value && (!this.fillGuard || this.fillGuard(element))) {
      this.setElementValue(element, autofilledValue);
    }
  ...
  private triggerFocusOnElement(element: HTMLElement | undefined, shouldResetValue = false): void {
    ...
    if (initialValue && "value" in element && (!this.fillGuard || this.fillGuard(element))) {
      this.setElementValue(element as FormFieldElement, initialValue);
    }
  }
