...
describe("AutofillService", () => {
  ...
  describe("getFormsWithPasswordFields", () => {
    ...
    it("returns an empty FormData array if no password fields are found", () => {
      jest.spyOn(AutofillScriptGenerator, "loadPasswordFields");

      const formData = autofillService.getFormsWithPasswordFields(pageDetailsMock);

      expect(AutofillScriptGenerator.loadPasswordFields).toHaveBeenCalledWith(
        pageDetailsMock,
        true,
        true,
        false,
    ...
    it("includes forms with password but no detectable username field with username: null", () => {
      ...
      jest.spyOn(AutofillScriptGenerator, "loadPasswordFields").mockReturnValueOnce([passwordInputField]);
  ...
  describe("generateFillScript", () => {
    ...
    describe("given a valid set of cipher fields and page detail fields", () => {
      it("will not attempt to fill by opid duplicate fields found within the page details", async () => {
        ...
        jest.spyOn(AutofillScriptGenerator, "fillByOpid");

        await autofillService["generateFillScript"](pageDetail, generateFillScriptOptions);

        expect(AutofillScriptGenerator.fillByOpid).not.toHaveBeenCalledWith(
          expect.anything(),
      ...
      it("will fill by opid fields that are not viewable but are a `span` element", async () => {
        ...
        expect(AutofillScriptGenerator.fillByOpid).toHaveBeenNthCalledWith(
          1,
          expect.anything(),
          defaultUsernameField,
          defaultUsernameField.value,
  ...
  describe("generateTargetedFillScript", () => {
    ...
    it("fills newPassword fields with the cipher password when inlineMenuFillType is PasswordGeneration", async () => {
      ...
      expect(AutofillScriptGenerator.fillByOpid).toHaveBeenCalledWith(
        expect.anything(),
        newPasswordField,
        "generated-pass",
      );
  ...
  describe("generateLoginFillScript", () => {
    ...
    it("returns null if the cipher does not have login data", async () => {
      ...
      jest.spyOn(AutofillScriptGenerator, "fillByOpid");
      jest.spyOn(AutofillScriptGenerator, "setFillScriptForFocus");
      ...
      expect(AutofillScriptGenerator.loadPasswordFields).not.toHaveBeenCalled();
      ...
      expect(AutofillScriptGenerator.fillByOpid).not.toHaveBeenCalled();
      expect(AutofillScriptGenerator.setFillScriptForFocus).not.toHaveBeenCalled();
      expect(value).toBeNull();
    });
    ...
    describe("given a valid set of page details and autofill options", () => {
      ...
      it("attempts to load the password fields from hidden and read only elements if no visible password fields are found within the page details", async () => {
        ...
        expect(AutofillScriptGenerator.loadPasswordFields).toHaveBeenCalledTimes(1);
        expect(AutofillScriptGenerator.loadPasswordFields).toHaveBeenCalledWith(
          pageDetails,
          false,
          false,
      ...
      describe("given a set of page details that does not contain a password field", () => {
        ...
        it("will attempt to keyword match a username to a viewable text, email or tel field if no password fields are found and the username fill is not being skipped", async () => {
          ...
          expect(AutofillScriptGenerator.fillByOpid).toHaveBeenCalledTimes(2);
          expect(AutofillScriptGenerator.fillByOpid).toHaveBeenCalledWith(
            fillScript,
            usernameField,
        ...
        it("classifies a field matching TotpFieldNames as TOTP", async () => {
          ...
          expect(AutofillScriptGenerator.fillByOpid).toHaveBeenCalledWith(fillScript, totpField, totpCode);
        });
        ...
        it("classifies a field matching AmbiguousTotpFieldNames as username when it also matches username signals", async () => {
          ...
          expect(AutofillScriptGenerator.fillByOpid).toHaveBeenCalledTimes(1);
          expect(AutofillScriptGenerator.fillByOpid).toHaveBeenCalledWith(
            fillScript,
            ambiguousField,
            options.cipher.login.username,
        ...
        it("classifies a field matching TotpFieldNames as TOTP even when it also matches username signals", async () => {
          ...
          expect(AutofillScriptGenerator.fillByOpid).toHaveBeenCalledWith(fillScript, field, totpCode);
          expect(AutofillScriptGenerator.fillByOpid).not.toHaveBeenCalledWith(
            fillScript,
            field,
            options.cipher.login.username,
      ...
      it("returns a fill script used to autofill a login item", async () => {
        ...
        expect(AutofillScriptGenerator.setFillScriptForFocus).toHaveBeenCalledWith(
          filledFields,
          fillScript,
        );
  ...
  describe("generateCardFillScript", () => {
    ...
    describe("given an invalid autofill field", () => {
      ...
      it("returns an unmodified fill script when the field is a `span` field", async () => {
        ...
        jest.spyOn(AutofillScriptGenerator, "isExcludedFieldType");
      ...
      it("returns an unmodified fill script when the field is not viewable", async () => {
        ...
        jest.spyOn(AutofillScriptGenerator, "forCustomFieldsOnly");
        jest.spyOn(AutofillScriptGenerator, "isExcludedFieldType");
        ...
        expect(AutofillScriptGenerator.forCustomFieldsOnly).toHaveBeenCalledWith(notViewableField);
        expect(AutofillService["isExcludedFieldType"]).toHaveBeenCalled();
        expect(value).toStrictEqual(unmodifiedFillScriptValues);
      });
    });
    ...
    describe("given a valid set of autofill fields", () => {
      ...
      beforeEach(() => {
        ...
        jest.spyOn(AutofillScriptGenerator, "forCustomFieldsOnly");
        jest.spyOn(AutofillScriptGenerator, "isExcludedFieldType");
        jest.spyOn(AutofillScriptGenerator as any, "isFieldMatch");
        jest.spyOn(autofillService as any, "makeScriptAction");
        jest.spyOn(AutofillScriptGenerator, "hasValue");
      ...
      it("returns a fill script containing all of the passed card fields", async () => {
        ...
        expect(AutofillScriptGenerator.forCustomFieldsOnly).toHaveBeenCalledTimes(6);
        expect(AutofillService["isExcludedFieldType"]).toHaveBeenCalledTimes(6);
        expect(AutofillScriptGenerator["isFieldMatch"]).toHaveBeenCalled();
      ...
      for (let index = 0; index < 2; index++) {
        describe(`when the isRefactorFeatureFlagSet is ${isRefactorFeatureFlagSet}`, () => {
          ...
          it("will not attempt to match custom fields", async () => {
            ...
            expect(AutofillScriptGenerator.forCustomFieldsOnly).toHaveBeenCalledWith(customField);
            expect(AutofillService["isExcludedFieldType"]).toHaveBeenCalled();
          ...
          it("will not attempt to match a field that is of an excluded type", async () => {
            ...
            expect(AutofillScriptGenerator.forCustomFieldsOnly).toHaveBeenCalledWith(excludedField);
            expect(AutofillService["isFieldMatch"]).not.toHaveBeenCalled();
            expect(value.script).toStrictEqual([]);
          });
          ...
          it("will not attempt to match a field that is not viewable", async () => {
            ...
            expect(AutofillScriptGenerator.forCustomFieldsOnly).toHaveBeenCalledWith(viewableField);
            expect(AutofillService["isExcludedFieldType"]).toHaveBeenCalled();
            expect(AutofillService["isFieldMatch"]).not.toHaveBeenCalled();
            expect(value.script).toStrictEqual([]);
          });
  ...
  describe("makeScriptActionWithValue", () => {
    ...
    it("will not add an autofill action to the fill script if the value does not exist", () => {
      ...
      expect(AutofillScriptGenerator.hasValue).toHaveBeenCalledWith(dataValue);
      expect(AutofillScriptGenerator.fillByOpid).not.toHaveBeenCalled();
    });
  ...
  describe("loadPasswordFields", () => {
    ...
    describe("AutofillScriptGenerator.autoCompleteTypeIncludesToken", () => {
    ...
    it("returns an empty array if passed a field that is a `span` element", () => {
      ...
      const result = AutofillScriptGenerator.loadPasswordFields(pageDetails, false, false, false, false);
    ...
    describe("given a field that is readonly", () => {
      ...
      it("returns the field within an array if the field can be readonly", () => {
        ...
        const result = AutofillScriptGenerator.loadPasswordFields(pageDetails, false, true, false, true);
    ...
    describe("given a field that is not viewable", () => {
      ...
      it("returns the field within an array if the field can be hidden", () => {
        ...
        const result = AutofillScriptGenerator.loadPasswordFields(pageDetails, true, false, false, true);
    ...
    describe("given a need for the passed to be empty", () => {
      it("returns an empty array if the passed field contains a value that is not null or empty", () => {
        ...
        const result = AutofillScriptGenerator.loadPasswordFields(pageDetails, false, false, true, false);
    ...
    describe("change password sibling exclusion, current-password token in same form", () => {
      ...
      it.each([
        ...
        (currentAc, middleAc) => {
          ...
          const result = AutofillScriptGenerator.loadPasswordFields(
            pageDetails,
            false,
            false,
            false,
    ...
    it("includes a lone password field with autocomplete off", () => {
      ...
      expect(
        AutofillScriptGenerator.loadPasswordFields(pageDetails, false, false, false, false),
    ...
    describe("given a field with a new password", () => {
      ...
      it("returns the field within an array if filling a new password and the autoCompleteType is `new-password`", () => {
        ...
        const result = AutofillScriptGenerator.loadPasswordFields(pageDetails, false, false, false, true);
  ...
  describe("findTotpField", () => {
    ...
    it("returns null when passed a field that is a `span` element", () => {
      ...
      expect(AutofillScriptGenerator.forCustomFieldsOnly).toHaveBeenCalledWith(field);
      expect(result).toBe(null);
    });
  ...
  describe("fieldPropertyIsMatch", () => {
    ...
    it("returns false if the property within the field does not have a value", () => {
      ...
      expect(AutofillScriptGenerator.hasValue).toHaveBeenCalledWith("");
      expect(result).toBe(false);
    });
    ...
    it("returns true if the property within the field provides a value that is equal to the passed `name`", () => {
      ...
      expect(AutofillScriptGenerator.hasValue).toHaveBeenCalledWith("some-value");
      expect(result).toBe(true);
    });
  ...
  describe("hasValue", () => {
    it("returns false if the passed string is null", () => {
      const result = AutofillScriptGenerator.hasValue(null);

      expect(result).toBe(false);
    });

    it("returns false if the passed string is an empty string", () => {
      const result = AutofillScriptGenerator.hasValue("");

      expect(result).toBe(false);
    });

    it("returns true if the passed string is not null or an empty string", () => {
      const result = AutofillScriptGenerator.hasValue("some-value");
  ...
  describe("setFillScriptForFocus", () => {
    ...
    it("returns a fill script with an unmodified actions list if an empty filledFields value is passed", () => {
      const result = AutofillScriptGenerator.setFillScriptForFocus({}, fillScript);
    ...
    it("returns a fill script with the password field prioritized when adding a `focus_by_opid` action", () => {
      const result = AutofillScriptGenerator.setFillScriptForFocus(filledFields, fillScript);
  ...
  describe("fillByOpid", () => {
    ...
    it("returns a list of fill script actions for the passed field", () => {
      ...
      AutofillScriptGenerator.fillByOpid(fillScript, usernameField, "some-long-value");
  ...
  describe("forCustomFieldsOnly", () => {
    it("returns a true value if the passed field has a tag name of `span`", () => {
      ...
      const result = AutofillScriptGenerator.forCustomFieldsOnly(field);
