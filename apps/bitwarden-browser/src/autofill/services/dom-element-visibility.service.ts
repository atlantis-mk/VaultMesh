...
class DomElementVisibilityService implements DomElementVisibilityServiceInterface {
  ...
  async isElementViewable(element: HTMLElement): Promise<boolean> {
    return this.isElementViewableNow(element);
  }

  /** Same visibility algorithm for the synchronous final assignment guard. */
  isElementViewableNow(element: HTMLElement): boolean {
