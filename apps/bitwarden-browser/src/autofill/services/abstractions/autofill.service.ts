...
export interface GenerateFillScriptOptions {
  /** Remote-vault planning only: fixed public marker, never a seed or actual OTP. */
  remoteTotpPlanning?: boolean;
  /** Return symbolic card/identity sources, never values or value-dependent formatting. */
  remoteItemPlanning?: boolean;
  /** Remote Email OTP planning uses only its public length; no code is hydrated. */
  remoteTotpLength?: 4 | 5 | 6 | 7 | 8;
  skipUsernameOnlyFill: boolean;
  onlyEmptyFields: boolean;
  fillNewPassword: boolean;
  allowTotpAutofill: boolean;
...
export abstract class AutofillService {
  /** Stateless script planning; remote adapters provide symbolic, not Vault, values. */
  generateFillScript!: (details: AutofillPageDetails, options: GenerateFillScriptOptions) => Promise<AutofillScript | null>;
