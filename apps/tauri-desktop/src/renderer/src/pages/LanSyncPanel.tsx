...
const labels: Record<LanSyncStatus['peers'][number]['state'], string> = {
  disabled: '自动同步已关闭', offline: '等待对端上线', 'waiting-unlock': '等待解锁',
  syncing: '正在同步', delivered: '密文已送达，等待对端合并', synced: '已合并', failed: '同步失败，可重试',
};
...
export function LanSyncPanel({ trusted }: { trusted: LanTrustedPeer[] }) {
  ...
  return <Card aria-label="局域网自动同步">
    <CardHeader><CardTitle>自动同步</CardTitle><CardDescription>已授权设备在同一局域网自动推送变化，锁定时只收发密文，桌面或插件解锁后合并；离开此页面仍继续。首次连接须双方解锁。各设备保留自己的主密码。邮件授权和设备权限保持本地；旧 Passkey 仅在本机使用。</CardDescription></CardHeader>
